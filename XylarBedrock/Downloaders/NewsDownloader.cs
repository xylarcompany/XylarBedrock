    ...
    public static class NewsDownloader
    {
        ...
        public static async Task UpdateLauncherFeed(NewsViewModel viewModel)
        {
            await Application.Current.Dispatcher.InvokeAsync(() =>
            {
                ...
                string latest_name = XylarBedrock.Localization.Language.LanguageManager.GetResource("LauncherNewsPage_Title_Text").ToString();
                foreach (var item in MainDataModel.Updater.Notes)
                {
                    ...
                    newItem.isBeta = item.url.Contains(XylarBedrock.Core.GithubAPI.BETA_URL);
